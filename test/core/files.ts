...
describe('File operations', function () {
  ...
  describe('symlink attack protection', function () {
    it('should not follow symlinks when writing files', async function () {
      const fs = await import('fs/promises');
      const os = await import('os');
      const tmpDir = await fs.mkdtemp(path.join(os.tmpdir(), 'clasp-symlink-test-'));
      const projectDir = path.join(tmpDir, 'project');
      const sensitiveFile = path.join(tmpDir, 'sensitive.txt');

      await fs.mkdir(projectDir, {recursive: true});
      await fs.writeFile(sensitiveFile, 'original-sensitive-data');

      // Create a symlink in project dir pointing to sensitive file
      const symlinkPath = path.join(projectDir, 'Code.js');
      await fs.symlink(sensitiveFile, symlinkPath);

      // Mock the project setup
      mockfs({
        [projectDir]: {
          'appsscript.json': '{"timeZone":"America/New_York","dependencies":{},"exceptionLogging":"STACKDRIVER"}',
          'Code.js': mockfs.symlink({path: sensitiveFile}),
        },
        [path.resolve(os.homedir(), '.clasprc.json')]: mockfs.load(
          path.resolve(__dirname, '../fixtures/dot-clasprc-authenticated.json'),
        ),
        '.clasp.json': JSON.stringify({scriptId: 'test-script-id', rootDir: projectDir}),
      }, {createCwd: true});
      ...
      // Attempt to pull files - this should NOT write to the symlink target
      const mockFiles = [{
        name: 'Code',
        type: 'SERVER_JS',
        source: 'attacker-controlled-content',
        localPath: 'Code.js',
      }];

      // @ts-ignore - accessing private method for testing
      await clasp.files.WriteFiles(mockFiles, projectDir);

      // Verify sensitive file was NOT modified
      const sensitiveContent = await fs.readFile(sensitiveFile, 'utf8');
      expect(sensitiveContent).to.equal('original-sensitive-data');

      // Cleanup
      await fs.rm(tmpDir, {recursive: true, force: true});
    });

    it('should block path traversal via relative paths', async function () {
      const fs = await import('fs/promises');
      const os = await import('os');
      const projectDir = path.join(os.tmpdir(), 'clasp-traversal-test');
      ...
      mockfs({
        ...
        '.clasp.json': JSON.stringify({scriptId: 'test-script-id'}),
      }, {createCwd: true});
      ...
      // @ts-ignore - accessing private method for testing
      await clasp.files.WriteFiles(maliciousFiles, projectDir);

      // File outside project dir should not exist
      try {
        await fs.access(path.join(projectDir, '../outside.js'));
        expect.fail('File should not have been written outside project dir');
      } catch {
