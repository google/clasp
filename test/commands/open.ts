...
describe('Test clasp open function', () => {
  ...
  it('open credentials setup page correctly', () => {
    const result = spawnSync(CLASP, ['open', '--creds-setup'], {encoding: 'utf8'});
    expect(result.stdout).to.contain(LOG.OPEN_CREDS_SETUP(PROJECT_ID));
  });
