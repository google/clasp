 ...
 * @return {boolean}
 */
export const hasOauthClientSettings = (
  local = false,
): boolean => (local ? fs.existsSync(DOT.RC.ABSOLUTE_LOCAL_PATH) : fs.existsSync(DOT.RC.ABSOLUTE_PATH));
...
export function getOAuthSettings(local: boolean): Promise<ClaspToken> {
  ...
  return RC
    .read<ClaspToken>()
    .catch((error) => logError(error, ERROR.NO_CREDENTIALS(local)));
}
...
Forgot ${PROJECT_NAME} commands? Get help:\n  ${PROJECT_NAME} --help`,
  ...
  NO_API: (enable: boolean, api: string) => `API ${api} doesn't exist. Try 'clasp apis ${enable ? 'enable' : 'disable'} sheets'.`,
...
Cloned ${fileNum} ${pluralize('files', fileNum)}.`,
  ...
  CREATE_DRIVE_FILE_FINISH: (filetype: string, fileid: string) => `Created new ${getFileTypeName(filetype) || '(unknown type)'}: ${URL.DRIVE(fileid)}`,
  CREATE_DRIVE_FILE_START: (filetype: string) => `Creating new ${getFileTypeName(filetype) || '(unknown type)'}...`,
  CREATE_PROJECT_FINISH: (filetype: string, scriptId: string) => `Created new ${getScriptTypeName(filetype)} script: ${URL.SCRIPT(scriptId)}`,
...
(including "project-id-")`,
  ...
  VERSION_DESCRIPTION: ({ versionNumber, description }: script_v1.Schema$Version) => `${versionNumber} - ${description || '(no description)'}`,
...
export function getWebApplicationURL(deployment: script_v1.Schema$Deployment) {
  ...
  const webEntryPoint = entryPoints.find(
    (entryPoint: script_v1.Schema$EntryPoint) => entryPoint.entryPointType === 'WEB_APP',
  );
